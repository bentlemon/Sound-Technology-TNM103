...
void loop()
{
  ...
  bufferIndex += badc1; // Samplingsfrekvens
  bufferIndex %= 512;
  ...
  // Flanger - Tar signalen och duplicerar, samt fasförskjuter den lite 
  // Ampliferar vissa frekvenser men tar bort andra
  soundSampleFromADC = badc0;
  sramBuffer[bufferIndex1] = soundSampleFromADC;
  soundSampleFromSramBuffer = sramBuffer[bufferIndex2];
  ++bufferIndex1;
  bufferIndex1 %= 512;
  bufferIndex2 = (bufferIndex1 - 2 * badc1) & 511;
  sramBufferSampleValue = ((soundSampleFromADC - 127) + (soundSampleFromSramBuffer - 127)) / 2 + 127;

  // Ringmodulation EJ KLAR
  /* soundSampleFromADC = badc0 - 127;
  int sampleValue1 = sramBuffer[bufferIndex1] - 127;
  int mixedValue = (sampleValue1 * soundSampleFromADC) / 1023; // skala om till 8 bitars tal
  sramBufferSampleValue = mixedValue + 127; 
  */
