...
float freqBuffer2 = 0.5; // Tonhöjare 
...
void loop() {
  ...
  if (badc1 == 0) {
    ...
    bufferIndex += badc1;
    bufferIndex2 += badc1 * freqBuffer2;
  }
  ...
  int sampleValue1 = sramBuffer[bufferIndex];
  int sampleValue2 = sramBuffer[bufferIndex2];
  

  sramBufferSampleValue = sramBuffer[bufferIndex];
  int mixedValue = (sampleValue1 + sampleValue2) / 2;
  ...
  //Serial.println(mixedValue); --> KAN INTE HÖRA LJUDET OM DENNA ÄR IGÅNG
...
void fillSramBufferWithWaveTable() {
  ...
  float deltaOfsoundValue = (2 * M_PI) / sizeof(sramBuffer);  // För sinusvågen
  float soundValueSine = 0.0; // För addering av sinus och sågtandsvågen
  float soundValueSaw = 0.0;  // För addering av sinus och sågtandsvågen

  // ---- FYRKANTSVÅG ----
  /*
  for (int i = 0; i < sizeof(sramBuffer) - 1; i++) {
    if (i > 107) {
      soundValue = 192.0;  //Vågformen är hög men varför inte 255?
    } else {
      soundValue = 64.0;  //Vågform är låg men inte 0? 
    }
  ...
  */
  ...
  for(int i = 256; i < (sizeof(sramBuffer)); i++) {
    soundValue -= 0.5;
    sramBuffer[i] = round(soundValue);
    Serial.println(soundValue);
  }
  ...
  */
  ...
  // ---- SINUSVÅG + SÅGTANDSVÅG ----
  /*
  for (int i = 0; i < sizeof(sramBuffer) - 1; i++) {
    float sinusSample = 127.0 * sin(soundValueSine) + 127.0; 

    soundValueSine += deltaOfsoundValue;

    soundValueSaw += 0.5;

    if (soundValueSaw >= 255.0) {
      soundValueSaw = 0.0;
    }

    float mixedValue = (sinusSample + soundValueSaw)/2;

    sramBuffer[i] = mixedValue;
    Serial.println(mixedValue);
  }
  */
}
